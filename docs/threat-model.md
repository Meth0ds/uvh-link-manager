...
| 19 | **Destino malicioso** (phishing/malware que acaba listando el dominio) | Denylist local de destinos aplicada en la vía de escritura (incluido *fallback* y reglas); reputación opcional asíncrona con auto-bloqueo desactivado por defecto; `unknown` nunca es `safe`; apelación con retirada de entradas. | `DestinationReputationTest`, `LinkAppealTest` |
