...
- [x] Cada timeout de job es menor que el timeout de su worker y éste es menor que el `retry_after` de la cola (`REDIS_QUEUE_RETRY_AFTER` con Redis, `DB_QUEUE_RETRY_AFTER` con la base).
  Jerarquía reparada y sostenida por un contrato el 2026-09-20: `queue-security` pasa de 60 s a 90 s porque su job más largo declara 60 s, y `QueueTimeoutContractTest` compara las tres fuentes —`docker-compose.production.yml`, los `$timeout` de `app/Jobs` y la ventana declarada en el compose y en la plantilla de entorno, con el defecto del framework (90 s) por debajo de los 180 s del worker de exportaciones— y falla si alguna deja de anidarse. **Y el export máximo que faltaba ya está demostrado** el mismo día en `frontend/e2e/async/drills/export-crash.mjs` (`exportCrashDrill`): con un export de tamaño máximo sembrado y ajustado contra el tope real (12.055.200 B de JSON codificado, 527.712 B por debajo del tope), el worker se mata con SIGKILL a mitad del trabajo y el ensayo comprueba que la fila —no el proceso— es el ancla, que el intento muerto no publica ningún bearer, que la reejecución publica **un único** artefacto y lo reemplaza en vez de añadir un segundo, que la descarga completa sirve el documento entero, que una descarga interrumpida no retira nada, que el acuse retira el artefacto y que el bearer retirado no se puede reutilizar; después mata al worker otra vez y deja la solicitud abandonada a la recuperación programada, que la termina sin huérfanos ni publicación.
...
- [ ] Export de acceso/portabilidad asíncrono, cifrado, de un solo uso, limitado al sujeto y sin secretos ni datos indebidos de otros miembros.
  Evidencia 2026-09-20: el ensayo asíncrono (**125/125**) recorre solicitud con step-up, confirmación con un bearer distinto, generación por el worker de exportaciones, descarga única y retirada del artefacto tras el acuse; `DataExportDownloadLifecycleTest` cubre los caminos de un solo uso.
