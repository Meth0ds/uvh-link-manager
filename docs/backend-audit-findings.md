...
- Corrección aplicada: `SessionManager::create` normaliza el header con una política explícita de persistencia (`persistableUserAgent`): secuencias UTF-8 inválidas purgadas, bytes de control —incluido NUL, que PostgreSQL rechaza dentro de `text`— eliminados y corte a 255 bytes sobre frontera de carácter (siempre cabe en `varchar(255)`); sin nada persistable se guarda `null`. `SessionUserAgentTest` cubre login, `mfa/verify` y `mfa/recovery` con `User-Agent` sobredimensionado y con UTF-8 inválido.
