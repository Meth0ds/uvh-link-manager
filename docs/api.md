...
| ------ | ---- | ---- | ----------- |
| POST | `/register` | — | Registro multistep. Requiere `name`, `email`, `password`, `captchaToken`, `acceptTerms: true`, `termsVersion` y `privacyVersion`; el honeypot `website` debe estar vacío. Crea usuario + workspace + cuota, audita por separado la aceptación contractual y el aviso de privacidad mostrado, y envía email de verificación sin crear sesión. La contraseña es una **propuesta**: no activa la cuenta ni autoriza moverla. Un registro anónimo sobre una dirección ya inscrita no la sustituye (ni su nombre, ni su workspace, ni su evidencia contractual, ni su bearer); todos los desenlaces contestan igual y emiten la cookie `REGISTRATION_EDIT_COOKIE`. |
...
| POST | `/verify-email` | — | `{ token, password, name, acceptTerms, termsVersion, privacyVersion }` → fija la identidad definitiva (nombre), registra la aceptación de las versiones legales vigentes con la fecha de ESTA activación, fija la contraseña definitiva de la cuenta, verifica el email, consume el bearer token, renombra el workspace autogenerado si aún lleva el nombre propuesto en el registro y revoca cualquier sesión legacy/preexistente; el acceso posterior exige un login nuevo. Todo lo que el registro anónimo propuso —nombre, workspace, evidencia contractual y contraseña— es provisional y muere aquí: lo decide quien abre el buzón tras la prueba de posesión (anti pre-hijack). |
