...
- El único sitio donde un factor viaja en claro es el comando de desarrollo `php artisan uvh:dev:totp <email>`, que imprime el código vigente de una cuenta (`--show-secret` añade el secreto y la URI de alta, `--watch` lo emite por intervalo). **Falla cerrado** fuera de `local`/`testing` y no tiene equivalente HTTP: un endpoint que devuelve un factor sería una superficie mucho mayor que un comando que ya exige el acceso capaz de leer la columna. Existe para ejercitar las pantallas reales —login, `/auth/reauthenticate` y el alta de MFA— sin un autenticador físico; su prueba fija las dos propiedades (el código lo acepta el verificador real y no se emite nada fuera de desarrollo).
