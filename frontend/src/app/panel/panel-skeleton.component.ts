...
@Component({
  ...
  styles: `
    ...
    .sk { position: relative; display: block; overflow: hidden; border-radius: 3px; background: color-mix(in srgb, var(--uvh-muted) 11%, var(--uvh-surface-raised)); }
    .sk::after { content: ""; position: absolute; inset: 0; transform: translateX(-105%); background: linear-gradient(90deg, transparent, color-mix(in srgb, var(--uvh-surface-raised) 65%, transparent), transparent); animation: shimmer 1.55s infinite cubic-bezier(.4,0,.2,1); }
    .metric-grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 14px; }
    .metric { height: 112px; border: 1px solid var(--uvh-border); }
    .chart { height: 300px; border: 1px solid var(--uvh-border); border-radius: 3px; }
    .split { display: grid; grid-template-columns: 1.15fr .85fr; gap: 16px; }
    .panel { height: 250px; border: 1px solid var(--uvh-border); border-radius: 3px; }
    .cards-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 16px; }
    .card-item { display: grid; height: 178px; padding: 22px; border: 1px solid var(--uvh-border); border-radius: 3px; }
    .card-item i { width: 40px; height: 40px; border-radius: 3px; background: color-mix(in srgb, var(--uvh-muted) 12%, transparent); }
    .card-item b { width: 42%; height: 16px; border-radius: 3px; background: color-mix(in srgb, var(--uvh-muted) 12%, transparent); }
    .card-item small { width: 74%; height: 11px; border-radius: 3px; background: color-mix(in srgb, var(--uvh-muted) 10%, transparent); }
    .form-card, .list-card { padding: 22px; border: 1px solid var(--uvh-border); border-radius: 3px; background: var(--uvh-surface-raised); }
    ...
    .avatar { width: 38px; height: 38px; border-radius: 3px; }
    .action { width: 76px; height: 34px; }
    @keyframes shimmer { to { transform: translateX(105%); } }
    @container (max-width: 860px) { .metric-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); } .split, .cards-grid { grid-template-columns: 1fr; } }
    @container (max-width: 520px) { .metric-grid { grid-template-columns: 1fr 1fr; gap: 10px; } .metric { height: 94px; } .chart { height: 230px; } .list-row { grid-template-columns: 34px minmax(0, 1fr); } .list-row .action { display: none; } .field.short { width: 100%; } }
