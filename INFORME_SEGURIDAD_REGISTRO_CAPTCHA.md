...
3. **Anti-enumeración sólido:** para emails ya registrados responde byte a byte igual que un alta nueva (`201` + `{"user":null}`), paga **un bcrypt en todas las ramas** (`Hash::make` de la contraseña propuesta antes de consultar la BD, más una admisión de outbox señuelo cuando el destino está ocupado), de modo que el coste no delate el desenlace; `login` iguala a su vez con `Hash::check` contra la constante `DUMMY_PASSWORD_HASH` cuando la cuenta no existe, y registra `auth.register_duplicate` en auditoría. Además, la condición de carrera (dos registros simultáneos del mismo email) se resuelve capturando el `23505` (unique violation) de PostgreSQL y devolviendo la misma respuesta.
...
6. **Cambio de email pre-verificación** (`change-registration-email`): exige captcha y la **cookie de edición** (`REGISTRATION_EDIT_COOKIE`) que emitió el `register` dueño de la fila pendiente —no la contraseña propuesta, que cualquier anónimo fabrica registrándose—; bloquea fila, borra tokens previos, revoca sesiones, rota la versión de seguridad (con lo que el secreto usado queda gastado) y mantiene el mismo contrato anti-enumeración: `403` genérico para todo desenlace que no sea el válido, y la misma admisión de outbox en ambos, que los jobs de entrega descartan cuando el destino ya está ocupado.
...
- Aplicada en **`register`** (con nombre+email del solicitante), **`reset-password`** y **`change-password`** (con nombre+email del usuario). Error genérico `422 "La contraseña es demasiado débil"` — sin detalle que ayude a ajustar ataques. NO aplicada en `change-registration-email` (esa ruta ya no recibe ni guarda ninguna contraseña: la autoriza la cookie de edición `REGISTRATION_EDIT_COOKIE`, cuyo secreto no se deduce de nada que el llamante escriba).
